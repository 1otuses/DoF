...
class SequenceDataset(torch.utils.data.Dataset): # 序列数据集，提供多智能体轨迹数据的加载、预处理和索引功能
    def __init__(
        ...
        fields = ReplayBuffer( # 初始化回放缓冲区
            n_agents,
            max_n_episodes, # 最大轨迹数
            max_path_length, # 最大轨迹长度
            termination_penalty, # 终止惩罚
            global_feats=self.global_feats, # 全局特征
            use_zero_padding=self.use_zero_padding, # 是否使用零填充
        )
        for _, episode in enumerate(itr):
            fields.add_path(episode)
        fields.finalize()

        self.normalizer = DatasetNormalizer( # 初始化数据集一 normalizer
            fields,
            normalizer,
    ...
    def __getitem__(self, idx: int, agent_idx: Optional[int] = None): # 获取数据集中的一个样本
        if self.agent_condition_type == "single": # 单体条件
        ...
        if self.pred_future_padding:
            loss_masks[self.history_horizon :] = 1.0 # 预测未来轨迹参与loss计算
        else:
        ...
        if self.include_returns:
            ...
            returns = np.array([returns / self.returns_scale], dtype=np.float32) # 对奖励进行缩放,归一化
