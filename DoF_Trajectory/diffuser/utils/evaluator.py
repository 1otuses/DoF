...
class MADEvaluatorWorker(Process):
    ...
    def _evaluate(self, load_step: Optional[int] = None):
        ...
        save_file_path = (
            ...
            else f"results/step_{load_step}-ep_{num_eval}-consistency_m.json"
            if getattr(Config, "use_consistency_models_sample", False)
    ...
    def _init(
        ...
        self.Config.use_return_to_go = getattr(Config, "use_return_to_go", True)
        self.Config.use_ddim_sample = getattr(Config, "use_ddim_sample", False)
        self.Config.use_consistency_models_sample = getattr(Config, "use_consistency_models_sample", False)
        ...
        if Config.use_consistency_models_sample:
            print(f"\n Use Consistency Models Sampler of {Config.n_consistency_models_steps} Step(s) \n")
            self.trainer.model.set_consistency_models_scheduler(Config.n_consistency_models_steps)
            self.trainer.ema_model.set_consistency_models_scheduler(Config.n_consistency_models_steps)
