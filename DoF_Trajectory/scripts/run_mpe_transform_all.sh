...
for map_name in "${MAPS[@]}"; do
  for quality in "${QUALITIES[@]}"; do
    echo "python scripts/transform_omar_dataset.py --env_name mpe --map_name ${map_name} --quality ${quality}"
    python scripts/transform_omar_dataset.py --env_name mpe --map_name "${map_name}" --quality "${quality}"
  done
done
